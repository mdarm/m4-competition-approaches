...
for m in model_arr:
    ...
    for series_length in training_lengths:
        ...
        # predict 
        #curr_prediction[:, horizon] = np.where(train_length_ok, est.predict(train_x).flatten(),
        #                                            curr_prediction[:, horizon])
        curr_prediction[:, :] = np.where(np.reshape(train_length_ok,(-1,1)), est.predict(train_x),
                                                    curr_prediction[:, :])
        
        
        # do the predictions 
        curr_valid_prediction[:,:] = np.where(np.reshape(valid_length_ok,(-1,1)), est.predict(valid_x),
                       curr_valid_prediction[:,:])
    ...
    if not (os.path.exists(os.path.join('multi_step/predictions',m.freq_name ))):
        os.makedirs(os.path.join('multi_step/predictions',m.freq_name ))
    ...
    for fc_type in fc_arr:
        ...
        # write output to csv 
        output.to_csv( os.path.join('multi_step/predictions',m.freq_name + '/' + 'Submission_{}_{}_{}.csv').format(fc_type, dt.datetime.now().strftime('%y%m%d_%H%M'),m.freq_name))
